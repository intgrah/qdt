...
public import IncrementalWF.FreeTheorem
...
abbrev Value (q : ℭ.Q) := { r : ℭ.R q // r = tasks.eval ι₀ q }
...
def run (q₀ : ℭ.Q)
  ...
  let ⟨r, hr⟩ ← MonadAttach.attach (tasks ι₀ q₀ (StateM (VCache tasks ι₀))
    (fun i => StateT.pure (ι₀ i))
    (fun q hq => Subtype.val <$> fetch q hq))
  have : r = tasks.eval ι₀ q₀ := by
    obtain ⟨s, s', heq⟩ := hr
