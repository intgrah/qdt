...
private def posExpr (p : String.Pos.Raw) : MacroM Term := do
  let n : Term := Syntax.mkNatLit p.byteIdx
  ``(String.Pos.Raw.mk $n)
...
private def nameFromBinder : Syntax → MacroM Name
  ...
  | .node _ ``Parser.Term.hole _ => return Name.anonymous
  | stx => Macro.throwErrorAt stx s!"expected binder name, got kind {stx.getKind}"

def level : Syntax → MacroM Term
  | .node _ ``Parser.Level.paren #[_, l, _] => level l
  | .node _ `null #[l] => level l
  | .ident _ _ n _ => ``(Universe.level $(quote n))
  | .node _ `num #[.atom _ raw] =>
      match raw.toNat? with
      | some 0 => ``(Universe.zero)
      | some n => ``(Nat.repeat Universe.succ $(Syntax.mkNatLit n) Universe.zero)
      | none => Macro.throwError s!"invalid level literal {raw}"
  | .node _ ``Parser.Level.num #[.node _ `num #[.atom _ raw]] =>
      match raw.toNat? with
      | some 0 => ``(Universe.zero)
      | some n => ``(Nat.repeat Universe.succ $(Syntax.mkNatLit n) Universe.zero)
      | none => Macro.throwError s!"invalid level literal {raw}"
  | .node _ ``Parser.Level.ident #[.ident _ _ n _] =>
      ``(Universe.level $(quote n))
  | .node _ ``Parser.Level.max #[_, .node _ `null #[l1, l2]] => do
      ``(Universe.max $(← level l1) $(← level l2))
  | .node _ ``Parser.Level.addLit #[l, _, .node _ `num #[.atom _ raw]] => do
      match raw.toNat? with
      | some n => ``(Nat.repeat Universe.succ $(Syntax.mkNatLit n) $(← level l))
      | none => Macro.throwError s!"invalid level offset {raw}"
  | stx => Macro.throwErrorAt stx s!"unsupported level syntax, kind {stx.getKind}"

private def levelListExpr (xs : List Term) : MacroM Term :=
  ``([$[$xs.toArray],*])

private def parseDeclIdUnivParams : Syntax → MacroM (List Name)
  | .node _ ``Parser.Command.declId #[_, univDecl] =>
      match univDecl with
      | .node _ `null #[] => return []
...
private def parseExplicitUniv : Syntax → MacroM (List Term)
  | .node _ `null #[] => return []
  | .node _ `null #[.node _ ``Parser.Term.explicitUniv #[_, .node _ `null levels, _]] =>
...
partial def term (stx : Syntax) : MacroM Term := do
  ...
  | .node _ ``Parser.Term.paren #[_, t, _] =>
      term t
  | .ident _ _ id _ =>
      ``(Cst.Term.ident $src $(quote id) [])
  | .node _ ``Parser.Term.explicitUniv #[.ident _ _ id _, _, .node _ `null levels, _] =>
  ...
  | .node _ `num #[.atom _ raw] =>
      match raw.toNat? with
      | some n =>
          let n : Term := Syntax.mkNatLit n
          ``(Cst.Term.natLit $src $n)
      | none =>
          Macro.throwErrorAt stx s!"invalid numeric literal {raw}"

  | .node _ ``Parser.Term.app #[f, .node _ `null args] => do
      args.foldlM
        (fun acc a => do ``(Cst.Term.app $src $acc $(← term a)))
        (← term f)

  | .node _ ``Parser.Term.fun #[_, .node _ ``Parser.Term.basicFun #[.node _ `null bs, tySpecs, _, body]] =>
      let body ← term body
      match tySpecs with
      | .node _ `null #[] =>
          let bs ← bs.mapM fun b => do
            ...
            | .node _ ``Parser.Term.hole _ => binderGroupUntypedExpr bSrc Name.anonymous
            | .node _ ``Parser.Term.typeAscription #[_, nameStx, _, .node _ `null #[ty], _] =>
      ...
      | .node _ `null #[.node _ ``Parser.Term.typeSpec #[_, ty]] =>
          let ty ← term ty
  ...
  | .node _ ``Parser.Term.arrow #[a, _, b] =>
      ``(Cst.Term.arrow $src $(← term a) $(← term b))

  | .node _ ``Parser.Term.depArrow #[binder, _, body] =>
      let binder ← match binder with
        | .node _ ``Parser.Term.explicitBinder #[_, .node _ `null names, .node _ `null #[_, ty], .node _ `null #[], _] =>
  ...
  | .node _ ``Parser.Term.forall #[_, .node _ `null binders, tySpecs, _, body] =>
      let body ← term body
      match tySpecs with
      | .node _ `null #[.node _ ``Parser.Term.typeSpec #[_, ty]] =>
          let ty ← term ty
      ...
      | .node _ `null #[] =>
          let groups ← binders.mapM fun b =>
            match b with
            | .node _ ``Parser.Term.explicitBinder #[_, .node _ `null names, .node _ `null #[_, ty], .node _ `null #[], _] => do
                let groupSrc ← srcFrom b
                let names ← names.mapM nameFromBinder
                let ty ← term ty
  ...
  | .node _ ``Parser.Term.typeAscription #[_, e, _, .node _ `null #[ty], _] =>
      ``(Cst.Term.ann $src $(← term e) $(← term ty))
  | .node _ ``Parser.Term.type #[_, .node _ `null #[]] =>
      ``(Cst.Term.u $src Universe.zero)
  | .node _ ``Parser.Term.type #[_, .node _ `null #[l]] =>
      ``(Cst.Term.u $src $(← level l))
  | .node _ ``Parser.Term.sorry _ =>
      ``(Cst.Term.sorry $src)
  ...
  | .node _ ``Parser.Term.let #[_, _, .node _ ``Parser.Term.letDecl #[.node _ ``Parser.Term.letIdDecl #[.node _ ``Parser.Term.letId #[.ident _ _ id _], .node _ `null #[], tySpecs, _, rhs]], _, body] =>
      let tyOpt ←
        match tySpecs with
        | .node _ `null #[] => ``(none)
        | .node _ `null #[.node _ ``Parser.Term.typeSpec #[_, ty]] =>
...
private def typedBinderGroupFromExplicitBinder (termFn : Syntax → MacroM Term) : Syntax → MacroM Term
  | stx@(.node _ ``Parser.Term.explicitBinder #[_, .node _ `null names, .node _ `null #[_, ty], .node _ `null #[], _]) => do
...
private def declIdName : Syntax → MacroM Name
  | .node _ ``Parser.Command.declId #[.ident _ _ id _, _] =>
      return id
  | stx =>
...
private def optDeclSigInfo (termFn : Syntax → MacroM Term) : Syntax → MacroM (List Term × Option Term)
  | .node _ ``Parser.Command.optDeclSig #[binders, tySpecs] => do
...
private def declSigInfo (termFn : Syntax → MacroM Term) : Syntax → MacroM (List Term × Term)
  | .node _ ``Parser.Command.declSig #[binders, .node _ ``Parser.Term.typeSpec #[_, ty]] => do
      let params ← typedBinderGroupsFromNullNode termFn binders
      let ty ← termFn ty
      return (params, ty)
  | stx =>
...
def cmd (stx : Syntax) : MacroM Term := do
  let src ← srcFrom stx
  match stx with
  | .node _ ``Parser.Command.declaration #[_, decl] =>
      cmd decl
  | .node _ ``Parser.Command.definition #[_, declId, optDeclSig, declVal, _] =>
      ...
      let rhs ←
        match declVal with
        | .node _ ``Parser.Command.declValSimple #[_, rhs, _, _] => pure rhs
        | _ => Macro.throwErrorAt declVal s!"unsupported def body, kind {declVal.getKind}"
      let body ← term rhs
      ``(Cst.Command.Cmd.definition ⟨$src, $(quote name), $(quote univParams), $params, $tyOpt, $body⟩)

  | .node _ ``Parser.Command.example #[_, optDeclSig, declVal] =>
      let (params, tyOpt?) ← optDeclSigInfo term optDeclSig
      let params ← termListExpr params
      let tyOpt ← optTermExpr tyOpt?
      let rhs ←
      ...
      let body ← term rhs
      ``(Cst.Command.Cmd.example ⟨$src, [], $params, $tyOpt, $body⟩)

  | .node _ ``Parser.Command.axiom #[_, declId, declSig] =>
      let name ← declIdName declId
      let univParams ← parseDeclIdUnivParams declId
      let (params, ty) ← declSigInfo term declSig
      let params ← termListExpr params
      ``(Cst.Command.Cmd.axiom ⟨$src, $(quote name), $(quote univParams), $params, $ty⟩)

  | .node _ ``Parser.Command.inductive #[_, declId, optDeclSig, _, .node _ `null ctors, _, _] =>
      let name ← declIdName declId
      let univParams ← parseDeclIdUnivParams declId
      let (params, tyOpt?) ← optDeclSigInfo term optDeclSig
      let params ← termListExpr params
      ...
      let ctors ← ctors.mapM fun ctorStx => do
        ...
        match ctorStx with
        | .node _ ``Parser.Command.ctor #[_, _, _, .ident _ _ id _, optDeclSig] =>
            let (cfields, ctyOpt?) ← optDeclSigInfo term optDeclSig
            let cfields ← termListExpr cfields
            let ctyOpt ← optTermExpr ctyOpt?
            ``(⟨$ctorSrc, $(quote id), $cfields, $ctyOpt⟩)
        | _ =>
            Macro.throwErrorAt ctorStx s!"unsupported constructor syntax, kind {ctorStx.getKind}"
      let ctors ← termListExpr ctors.toList
      ``(Cst.Command.Cmd.inductive ⟨$src, $(quote name), $(quote univParams), $params, $tyOpt, $ctors⟩)

  | .node _ ``Parser.Command.structure #[_, declId, optDeclSig, _, whereFields, _] =>
      let name ← declIdName declId
      let univParams ← parseDeclIdUnivParams declId
      let (params, tyOpt?) ← optDeclSigInfo term optDeclSig
      let params ← termListExpr params
      ...
      let fields ←
        ...
        | .node _ ``Parser.Command.structFields #[.node _ `null fs] =>
            let fieldsArray ← fs.mapM fun f => do
              let fSrc ← srcFrom f
              match f with
              | .node _ ``Parser.Command.structExplicitBinder #[_, _, .node _ `null #[idStx@(.ident _ _ id _)], optDeclSig, _, _] =>
                  let nameSrc ← srcFrom idStx
                  let (fparams, ftyOpt?) ← optDeclSigInfo term optDeclSig
                  let fparams ← termListExpr fparams
                  match ftyOpt? with
                  | some fty =>
                      ``(⟨$fSrc, $nameSrc, $(quote id), $fparams, $fty⟩)
                  | none =>
      ...
      ``(Cst.Command.Cmd.structure ⟨$src, $(quote name), $(quote univParams), $params, $tyOpt, $fields⟩)
