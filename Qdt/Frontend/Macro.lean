...
private def srcFrom : Syntax → MacroM Term
  | stx => do
    match stx.getPos?, stx.getTailPos? with
    | some startPos, some endPos => do
...
partial def term : Syntax → MacroM Term
  | stx => do
      let src ← srcFrom stx
      match stx with
      ...
      | .node _ `Lean.Parser.Term.fun #[_, .node _ `Lean.Parser.Term.basicFun #[.node _ `null bs, tySpecs, _, body]] =>
          ...
          | .node _ `null #[] => do
              let bs ← bs.mapM fun b => do
                ...
                | .node _ `Lean.Parser.Term.typeAscription #[_, nameStx, _, .node _ `null #[ty], _] => do
                    let groupSrc ← srcFrom b
                    let name ← nameFromBinder nameStx
                    let ty ← term ty
                    binderGroupTypedExpr (← typedBinderGroupExpr groupSrc [name] ty)
                | _ => Macro.throwErrorAt b s!"unsupported fun binder, kind {b.getKind}"
              `(Cst.Term.lam $src [$[$bs],*] $body)
          | .node _ `null #[.node _ `Lean.Parser.Term.typeSpec #[_, ty]] => do
              let ty ← term ty
              let bs ← bs.mapM nameFromBinder
              let groupSrc ← srcFrom tySpecs
              let typedGroup ← typedBinderGroupExpr groupSrc bs.toList ty
      ...
      | .node _ `Lean.Parser.Term.arrow #[a, _, b] => do
          let a ← term a
          let b ← term b
          `(Cst.Term.arrow $src $a $b)

      | .node _ `Lean.Parser.Term.depArrow #[binder, _, body] => do
          let binder ← match binder with
            | .node _ `Lean.Parser.Term.explicitBinder #[_, .node _ `null names, .node _ `null #[_, ty], .node _ `null #[], _] => do
                ...
                let ty ← term ty
                typedBinderGroupExpr binderSrc names.toList ty
            | _ => Macro.throwErrorAt binder s!"unsupported dependent arrow binder, kind {binder.getKind}"
          let body ← term body
          `(Cst.Term.pi $src $binder $body)

      | .node _ `Lean.Parser.Term.forall #[_, .node _ `null binders, tySpecs, _, body] => do
          let body ← term body
          match tySpecs with
          | .node _ `null #[.node _ `Lean.Parser.Term.typeSpec #[_, ty]] => do
              let ty ← term ty
              ...
              let groupSrc ← srcFrom tySpecs
              let group ← typedBinderGroupExpr groupSrc names.toList ty
              `(Cst.Term.pi $src $group $body)
          | .node _ `null #[] => do
              let groups ← binders.mapM fun b => do
                match b with
                | .node _ `Lean.Parser.Term.explicitBinder #[_, .node _ `null names, .node _ `null #[_, ty], .node _ `null #[], _] => do
                    let groupSrc ← srcFrom b
                    let names ← names.mapM nameFromBinder
                    let ty ← term ty
                    typedBinderGroupExpr groupSrc names.toList ty
      ...
      | .node _ `Lean.Parser.Term.typeAscription #[_, e, _, .node _ `null #[ty], _] => do
          let e ← term e
          let ty ← term ty
      ...
      | .node _ `Lean.Parser.Term.tuple #[_, .node _ `null items, _] => do
          let rec collect : Array Syntax → MacroM (Array Syntax)
            | #[t] => return #[t]
            | #[t, .atom _ ",", .node _ `null rest] => return #[t] ++ (← collect rest)
            | xs => Macro.throwErrorAt stx s!"unsupported tuple syntax, items size {xs.size}"
          let ts ← collect items
          if h : ts.size = 2 then
            let a ← term ts[0]
            let b ← term ts[1]
            `(Cst.Term.pair $src $a $b)
          else
            Macro.throwErrorAt stx "Only pairs supported"

      | .node _ k #[a, _, b] =>
          if k == Name.mkSimple "term_=_"
          then
            let a ← term a
            let b ← term b
            `(Cst.Term.eq $src $a $b)
          else if k == Name.mkSimple "term_+_"
          then
            let a ← term a
            let b ← term b
            `(Cst.Term.add $src $a $b)
          else if k == Name.mkSimple "term_-_"
          then
            let a ← term a
            let b ← term b
            `(Cst.Term.sub $src $a $b)
          else if k == Name.mkSimple "term_*_"
          then
            let a ← term a
            let b ← term b
            `(Cst.Term.mul $src $a $b)
          else if k == Name.mkSimple "term_×_"
          then
            let a ← term a
            let b ← term b
            `(Cst.Term.prod $src $a $b)
          else
            Macro.throwErrorAt stx s!"unsupported term kind {k}"

      | .node _ `Lean.Parser.Term.let #[_, _, .node _ `Lean.Parser.Term.letDecl #[.node _ `Lean.Parser.Term.letIdDecl #[.node _ `Lean.Parser.Term.letId #[.ident _ _ id _], .node _ `null #[], tySpecs, _, rhs]], _, body] => do
          let tyOpt ←
            match tySpecs with
            | .node _ `null #[] => `(none)
...
partial def cmd : Syntax → MacroM Term
  | stx => do
      let src ← srcFrom stx
      match stx with
      ...
      | .node _ `Lean.Parser.Command.inductive #[_, declId, optDeclSig, _, .node _ `null ctors, _, _] => do
          ...
          let ctors ← ctors.mapM fun ctorStx => do
            ...
            match ctorStx with
            | .node _ `Lean.Parser.Command.ctor #[_, _, _, .ident _ _ id _, optDeclSig] => do
      ...
      | .node _ `Lean.Parser.Command.structure #[_, declId, optDeclSig, _, whereFields, _] => do
          ...
          let fields ←
            ...
            | .node _ `Lean.Parser.Command.structFields #[.node _ `null fs] => do
                let fieldsArray ← fs.mapM fun f => do
                  let fSrc ← srcFrom f
                  match f with
                  | .node _ `Lean.Parser.Command.structExplicitBinder #[_, _, .node _ `null #[.ident _ _ id _], optDeclSig, _, _] => do
                      let (fparams, ftyOpt?) ← optDeclSigInfo term optDeclSig
                      let fparams ← termListExpr fparams
                      match ftyOpt? with
                      | some fty =>
                          `(Cst.Command.StructureField.mk $fSrc $(quote id) $fparams $fty)
                      | none =>
