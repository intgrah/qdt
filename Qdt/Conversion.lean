...
public partial def VTm.conv {n} (a b : VTm n) (cs : ConvState := .rigid) : ElabM ι₀ q₀ Bool :=
  ...
  | .glued ne₁ _, .glued ne₂ _ => do
      ...
      | _ =>
        ...
        let b' ← b.whnf ι₀ q₀
        a'.conv b' cs
  | .glued _ _, other => do (← a.whnf ι₀ q₀).conv other cs
  | other, .glued _ _ => do other.conv (← b.whnf ι₀ q₀) cs
  ...
  | .lam _ _ ⟨env, body⟩, other => do
      let var : VTm (n + 1) := VTm.varAt n
      let bVal ← body.eval ι₀ q₀ (env.weaken.cons var)
      match other.weaken (m := n + 1) with
      | .neutral ne =>
          let oVal ← (VTm.neutral ne).app ι₀ q₀ var
          bVal.conv oVal cs
      | _ => return false
  ...
  | .pi' _ a₁ ⟨env₁, b₁⟩, .pi' _ a₂ ⟨env₂, b₂⟩ => do
      ...
      let var : VTm (n + 1) := VTm.varAt n
      let b₁Val ← b₁.eval ι₀ q₀ (env₁.weaken.cons var)
      let b₂Val ← b₂.eval ι₀ q₀ (env₂.weaken.cons var)
      b₁Val.conv b₂Val cs
  | .neutral ne, other => do
      match ← (VTm.neutral ne).whnf ι₀ q₀ with
...
partial def etaConv {n} (ne : Neutral n) (other : VTm n) (cs : ConvState) : ElabM ι₀ q₀ Bool := do
  let ⟨.const ctorName _us, sp⟩ := ne
    | return false
  let some ctorInfo ← fetchConstructor ι₀ q₀ ctorName
    | return false
  ...
  List.finRange fields.length |>.allM (fun (i : Fin fields.length) => do
    let proj ← other.proj ι₀ q₀ i.val
    proj.conv fields[i] cs
  )

partial def Neutral.conv {n} : Neutral n → Neutral n → ConvState → ElabM ι₀ q₀ Bool
  | ne₁@⟨h₁, sp₁⟩, ne₂@⟨h₂, sp₂⟩, cs => do
      ...
      | .const n₁ us₁, .const n₂ us₂ =>
          if n₁ == n₂ && us₁ == us₂ then
            ...
            | .rigid =>
                ...
                match ← deltaReduction ι₀ q₀ n₁ us₁ with
                | some v₁ => (← applySpine ι₀ q₀ sp₁ v₁).conv (← applySpine ι₀ q₀ sp₂ v₁) .full
            ...
            | .flex => return false
            | _ =>
              match ← deltaReduction ι₀ q₀ n₁ us₁, ← deltaReduction ι₀ q₀ n₂ us₂ with
              | some v₁, some v₂ =>
                  (← applySpine ι₀ q₀ sp₁ v₁).conv (← applySpine ι₀ q₀ sp₂ v₂) .full
              | some v₁, none =>
                  (← applySpine ι₀ q₀ sp₁ v₁).conv (.neutral ne₂) .full
              | none, some v₂ =>
                  (VTm.neutral ne₁).conv (← applySpine ι₀ q₀ sp₂ v₂) .full
              | none, none =>
      ...
      | .const n₁ us₁, .var _ =>
          ...
          | _ =>
            match ← deltaReduction ι₀ q₀ n₁ us₁ with
            | some v₁ => (← applySpine ι₀ q₀ sp₁ v₁).conv (.neutral ne₂) .full
      ...
      | .var _, .const n₂ us₂ =>
          ...
          | _ =>
            match ← deltaReduction ι₀ q₀ n₂ us₂ with
            | some v₂ => (VTm.neutral ne₁).conv (← applySpine ι₀ q₀ sp₂ v₂) .full
            | none => etaConv ne₂ (.neutral ne₁) cs

partial def Spine.conv {n} : Spine n → Spine n → ConvState → ElabM ι₀ q₀ Bool
...
public partial def VTy.conv {n} (a b : VTy n) (cs : ConvState := .rigid) : ElabM ι₀ q₀ Bool :=
  match a, b with
  | .u i₁, .u i₂ => return i₁ == i₂
  | .pi _ a₁ ⟨env₁, b₁⟩, .pi _ a₂ ⟨env₂, b₂⟩ => do
      if !(← a₁.conv a₂ cs) then return false
