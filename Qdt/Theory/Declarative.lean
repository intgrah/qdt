...
inductive Ty.WF : Global → {n : Nat} → Ctx 0 n → Ty n → Type
  ...
  | el_form {Δ Γ i t} :
      (Δ; Γ ⊢ t : .u i) →
      (Δ; Γ ⊢ .el t type)
  | pi_form {Δ Γ x bi A B} :
      (Δ; Γ ⊢ A type) →
      (Δ; Γ.snoc ⟨x, bi, A⟩ ⊢ B type) →
      (Δ; Γ ⊢ .pi x bi A B type)
...
inductive Ty.Eq : Global → {n : Nat} → Ctx 0 n → Ty n → Ty n → Type
  ...
  | trans {Δ Γ A B C} :
      ...
      (Δ; Γ ⊢ B ≡ C type) →
      (Δ; Γ ⊢ A ≡ C type)
  | pi_form_eq {Δ Γ A₁ A₂ x x' bi bi' B₁ B₂} :
      (Δ; Γ ⊢ A₁ type) →
      (Δ; Γ ⊢ A₁ ≡ A₂ type) →
      (Δ; Γ.snoc ⟨x, bi, A₁⟩ ⊢ B₁ ≡ B₂ type) →
      (Δ; Γ ⊢ .pi x bi A₁ B₁ ≡ .pi x' bi' A₂ B₂ type)
...
inductive Tm.Eq : Global → {n : Nat} → Ctx 0 n → Tm n → Tm n → Ty n → Type
  ...
  | delta {Δ Γ name us info} :
      ...
      (Δ; Γ ⊢ .const name us
              ≡ (info.tm.instantiateParams info.univParams us).wkClosed
              : (info.ty.instantiateParams info.univParams us).wkClosed)
  | pi_intro_eq {Δ Γ x x' bi bi' b₁ b₂ A₁ A₂ B} :
      (Δ; Γ ⊢ A₁ type) →
      (Δ; Γ ⊢ A₁ ≡ A₂ type) →
      (Δ; Γ.snoc ⟨x, bi, A₁⟩ ⊢ b₁ ≡ b₂ : B) →
      (Δ; Γ ⊢ .lam x bi A₁ b₁ ≡ .lam x' bi' A₂ b₂ : .pi x bi A₁ B)
  | pi'_eq {Δ Γ x x' bi bi' A₁ A₂ B₁ B₂ i j} :
      (Δ; Γ ⊢ A₁ : .u i) →
      (Δ; Γ ⊢ A₁ ≡ A₂ : .u i) →
      (Δ; Γ.snoc ⟨x, bi, .el A₁⟩ ⊢ B₁ ≡ B₂ : .u j) →
      (Δ; Γ ⊢ .pi' x bi A₁ B₁ ≡ .pi' x' bi' A₂ B₂ : .u (i.mkMax j))
  | pi_elim_eq {Δ Γ x bi f₁ f₂ a₁ a₂ A B} :
      (Δ; Γ ⊢ f₁ ≡ f₂ : .pi x bi A B) →
      (Δ; Γ ⊢ a₁ ≡ a₂ : A) →
      (Δ; Γ ⊢ .app f₁ a₁ ≡ .app f₂ a₂ : B[a₁])
  | pi_comp {Δ Γ x bi a b A B} :
      (Δ; Γ ⊢ A type) →
      (Δ; Γ.snoc ⟨x, bi, A⟩ ⊢ b : B) →
      (Δ; Γ ⊢ a : A) →
      (Δ; Γ ⊢ .app (Tm.lam x bi A b) a ≡ b[a] : B[a])
  | pi_uniq {Δ Γ x bi A B f} :
      (Δ; Γ ⊢ f : .pi x bi A B) →
      (Δ; Γ ⊢ f ≡ .lam x bi A (.app (↑f) (.var 0)) : .pi x bi A B)
  | zeta {Δ Γ x A e body B} :
      (Δ; Γ ⊢ A type) →
      (Δ; Γ ⊢ e : A) →
      (Δ; Γ.snoc ⟨x, .explicit, A⟩ ⊢ body : B) →
  ...
  | iota {Δ n} {Γ : Ctx 0 n}
      ...
      (Δ; Γ ⊢ (rule.rhs.instantiateParams info.univParams recUs).subst
                 (Subst.fromArgs (params ++ motives ++ minors ++ fields) hArgs) : A) →
      (Δ; Γ ⊢ (Tm.const recName recUs).apps
                 (params ++ motives ++ minors ++ indices ++
                   [(Tm.const ctorName ctorUs).apps (ctorParams ++ fields)])
            ≡ (rule.rhs.instantiateParams info.univParams recUs).subst
...
inductive Tm.HasType : Global → {n : Nat} → Ctx 0 n → Tm n → Ty n → Type
  ...
  | const {Δ Γ name us info} :
      ...
      (us.length = info.numUnivParams) →
      (Δ; Γ ⊢ .const name us : (info.ty.instantiateParams info.univParams us).wkClosed)
  | pi_intro {Δ Γ x bi A body B} :
      (Δ; Γ ⊢ A type) →
      (Δ; Γ.snoc ⟨x, bi, A⟩ ⊢ body : B) →
      (Δ; Γ ⊢ .lam x bi A body : .pi x bi A B)
  | pi_elim {Δ Γ f a x bi A B} :
      (Δ; Γ ⊢ f : .pi x bi A B) →
      (Δ; Γ ⊢ a : A) →
      (Δ; Γ ⊢ .app f a : B[a])
  ...
  | pi' {Δ Γ x bi A B i j} :
      (Δ; Γ ⊢ A : .u i) →
      (Δ; Γ.snoc ⟨x, bi, .el A⟩ ⊢ B : .u j) →
      (Δ; Γ ⊢ .pi' x bi A B : .u (i.mkMax j))
