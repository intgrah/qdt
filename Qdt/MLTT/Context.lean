...
theorem Ctx.weaken_snoc {m n x A}
    (Γ : Ctx m n) :
    Ctx.weaken (Γ.snoc ⟨x, A⟩) = Nat.succ_add n 1 ▸ (Ctx.weaken Γ).snoc ⟨x, ↑A⟩ := by
  unfold Ctx.weaken
  rw [Tele.dmap_snoc]
...
def Ctx.subst {m} (a : Tm m) : {n : Nat} → Ctx (m + 1) (n + 1) → Ctx m n
  ...
  | n + 1, .snoc Γ ⟨x, B⟩ =>
      ...
      .snoc (Ctx.subst a Γ) ⟨x, B.subst (Subst.beta (heq ▸ a.subst (Subst.shiftN (n - m))))⟩
