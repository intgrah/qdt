...
public partial def shakeFetch
    ...
    (tasks : Tasks Monad ℭ)
    (ι₀ : ∀ i, ℭ.V i)
    (memos : ST.Ref σ (DHashMap ℭ.Q (Shake.Memo ℭ)))
    (started : ST.Ref σ (DHashMap ℭ.Q (Shake.Memo ℭ)))
    (q : ℭ.Q) : ST σ (ℭ.R q) := do
  ...
  let fetch' (q : ℭ.Q) : ST σ (ℭ.R q) := do
    let r ← shakeFetch tasks ι₀ memos started q
  ...
  let verifyDeps (deps : HashMap ℭ.Q UInt64) : ST σ Bool := do
    for (depKey, oldHash) in deps do
      let _ ← shakeFetch tasks ι₀ memos started depKey
...
@[expose] public def Shake [∀ q, Inhabited (ℭ.R q)] : Build Monad ℭ J where
  σ := Shake.Store ℭ J
  init inputs := {
    inputs
  ...
  build tasks q := fun store =>
    ...
    runST fun σ => do
      ...
      return (← shakeFetch tasks ι₀ memos started q, ⟨store.inputs, ← memos.get⟩)
